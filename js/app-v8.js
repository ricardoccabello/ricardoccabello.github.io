...
if(atlasElement){
  ...
  function renderMarkers(stage='all'){
    ...
    Object.entries(groups).forEach(([id,g],idx)=>{
      ...
      btn.innerHTML=`<span>${idx+1}</span>${g.keys.length>1?`<span class="count">${g.keys.length}</span>`:''}`;
      btn.title=g.keys.map(k=>getProject(k).title).join(' · ');
  ...
  function openProject(key,scrollToMap=false){
    ...
    drawer.classList.add('active'); document.querySelectorAll('.project-row').forEach(r=>r.classList.toggle('active',r.dataset.project===key));
  ...
  document.querySelector('.drawer-close')?.addEventListener('click',()=>{drawer.classList.remove('active');activeProjectKey=null;document.querySelectorAll('.project-row,.atlas-marker').forEach(e=>e.classList.remove('active'));});
