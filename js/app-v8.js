...
if(atlasElement){
  ...
  function openProject(key,scrollToMap=false){
    ...
    if(drawerGallery){
      ...
      drawerGallery.innerHTML=imgs.map(src=>`<div class="drawer-image protected-media"><img src="${src}" alt="${p.title}" draggable="false"></div>`).join('');
    }
    if(drawerLink){
  ...
  window.refreshAtlasLanguage=function(){renderMarkers(activeStage);if(activeProjectKey)openProject(activeProjectKey,false);};
  renderMarkers('all');
