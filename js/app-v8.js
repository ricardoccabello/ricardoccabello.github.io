...
if(atlasElement){
  ...
  document.querySelectorAll('.stage-button').forEach(b=>b.addEventListener('click',()=>{
    activeStage=b.dataset.stage;document.querySelectorAll('.stage-button').forEach(x=>x.classList.toggle('active',x===b));
    document.querySelectorAll('.project-group').forEach(g=>g.classList.toggle('hidden',activeStage!=='all'&&g.dataset.stageGroup!==activeStage));renderMarkers(activeStage);
  }));
