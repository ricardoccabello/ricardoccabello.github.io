...
/* 2026 mobile viewport stabilisation */
(()=>{
  const resetTransientUI=()=>{
    document.documentElement.classList.remove('page-leaving');
    document.body.classList.remove('page-leaving','nav-open');
    document.querySelector('.page-transition')?.classList.remove('active');
    document.querySelector('.page-loader')?.classList.add('loaded');
  };
  addEventListener('pageshow',event=>{resetTransientUI();if(event.persisted)requestAnimationFrame(()=>dispatchEvent(new Event('resize')))});
  addEventListener('pagehide',()=>document.body.classList.remove('nav-open'));
  let viewportTimer;
  const syncViewport=()=>{
    clearTimeout(viewportTimer);
    viewportTimer=setTimeout(()=>{
      document.documentElement.style.setProperty('--mobile-vh',`${(window.visualViewport?.height||innerHeight)*.01}px`);
      dispatchEvent(new Event('portfolio:viewportchange'));
    },60);
  };
